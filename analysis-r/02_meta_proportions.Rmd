...
```{r meta_prop_setup, include=FALSE}
...
# If we have a study_metadata table with publication years, join it
if (is.data.frame(deduplicated_data$study_metadata) && 
    nrow(deduplicated_data$study_metadata) > 0 && 
    "publication_year" %in% names(deduplicated_data$study_metadata)) {
  
  ssi_for_meta <- ssi_for_meta %>%
    left_join(select(deduplicated_data$study_metadata, study_id, publication_year), by = "study_id")
} else {
  # If no publication_year in metadata, try to get it from year_start in the original data
  if ("year_start" %in% names(ssi_for_meta)) {
    ssi_for_meta <- ssi_for_meta %>%
      mutate(publication_year = as.character(year_start))
  } else {
    # Add an NA column if no publication year data exists
    ssi_for_meta <- ssi_for_meta %>%
      mutate(publication_year = NA_character_)
  }
}
...
```
```{r ssi_preview}
cat("SSI rows (raw): ", nrow(ssi_data), " | complete for meta: ", k_ssi, "\n")
knitr::kable(head(ssi_for_meta %>% dplyr::select(study_id, numerator, denominator, dplyr::any_of(c("publication_year", "pub_year"))), 10))
```
...
if (k_ssi >= meta_params$min_studies) {
  ...
  forest(meta_ssi_results,
    ...
    print.tau2 = TRUE)
  grDevices::png(file.path(config$paths$plots_dir, "forest_ssi_incidence.png"), width = 10, height = 8, units = "in", res = 300)
  forest(meta_ssi_results, comb.label = "Random effects model", print.tau2 = TRUE)
  grDevices::dev.off()
  
  # --- Funnel Plot ---
  funnel(meta_ssi_results)
  grDevices::png(file.path(config$paths$plots_dir, "funnel_ssi_incidence.png"), width = 8, height = 6, units = "in", res = 300)
  funnel(meta_ssi_results)
  grDevices::dev.off()
...
if (k_ssi >= meta_params$min_studies && !all(is.na(ssi_for_meta$pub_year))) {
  ...
  grDevices::png(file.path(config$paths$plots_dir, "bubble_metareg_ssi.png"), width = 8, height = 6, units = "in", res = 300)
  bubble(metareg_ssi, xlab = "Publication Year", ylab = "SSI Proportion (Logit Scale)")
  grDevices::dev.off()
...
if (is.data.frame(deduplicated_data$study_metadata) && 
  ...
  mortality_for_meta <- mortality_for_meta %>%
    left_join(select(deduplicated_data$study_metadata, study_id, publication_year), by = "study_id")
} else {
  # If no publication_year in metadata, try to get it from year_start in the original data
  if ("year_start" %in% names(mortality_for_meta)) {
    mortality_for_meta <- mortality_for_meta %>%
      mutate(publication_year = as.character(year_start))
  } else {
    # Add an NA column if no publication year data exists
...
```
```{r mortality_preview}
cat("Mortality rows (raw): ", nrow(mortality_data), " | complete for meta: ", k_mortality, "\n")
knitr::kable(head(mortality_for_meta %>% dplyr::select(study_id, numerator, denominator, dplyr::any_of(c("publication_year", "pub_year"))), 10))
```
...
if (k_mortality >= meta_params$min_studies) {
  ...
  grDevices::png(file.path(config$paths$plots_dir, "forest_mortality.png"), width = 10, height = 8, units = "in", res = 300)
  forest(meta_mortality_results, comb.label = "Random effects model", print.tau2 = TRUE)
  grDevices::dev.off()
  
  # --- Funnel Plot ---
  funnel(meta_mortality_results)
  grDevices::png(file.path(config$paths$plots_dir, "funnel_mortality.png"), width = 8, height = 6, units = "in", res = 300)
  funnel(meta_mortality_results)
  grDevices::dev.off()
...
if (!"pathogen" %in% names(amr_data)) amr_data$pathogen <- NA_character_
if (!"antibiotic" %in% names(amr_data)) amr_data$antibiotic <- NA_character_
...
```
```{r amr_preview}
cat("AMR rows (raw): ", nrow(amr_data), " | eligible pairs: ", dplyr::n_distinct(amr_groups$pair), "\n")
knitr::kable(head(amr_for_meta %>% dplyr::select(study_id, pathogen, antibiotic, numerator, denominator), 10))
```
