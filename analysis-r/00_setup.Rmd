...
required_packages <- c(
  ...
  "readr", "tidyr", "tibble", "igraph", "meta", "metafor", "ggplot2", "knitr", "broom"
)
...
normalize_text_scalar <- function(text) {
...
normalize_text <- Vectorize(normalize_text_scalar, "text")

# --- Author List Processing (Vectorized) ---
normalize_authors_scalar <- function(author_string) {
  if (is.na(author_string) || author_string == "") return(list(character(0)))
...
normalize_authors <- function(author_vector) {
    lapply(author_vector, function(x) normalize_authors_scalar(x)[[1]])
}
...
### 6. Standardize Quantitative Dataset Shapes
Ensure expected numeric columns exist so downstream notebooks can rely on common names (numerator/denominator).

```{r standardize_shapes}
# Helper to ensure numerator/denominator columns exist by mapping from common alternatives
standardize_counts <- function(df, num_candidates, den_candidates) {
  # If df is not a data.frame/tibble (e.g., NULL), return as-is
  if (!inherits(df, "data.frame")) return(df)

  # Add numerator from the first available candidate
  if (!"numerator" %in% names(df)) {
    for (cand in num_candidates) {
      if (cand %in% names(df)) { df$numerator <- as.double(df[[cand]]); break }
    }
  }
  # Add denominator from the first available candidate
  if (!"denominator" %in% names(df)) {
    for (cand in den_candidates) {
      if (cand %in% names(df)) { df$denominator <- as.double(df[[cand]]); break }
    }
  }
  # If still missing, create NA columns of correct type
  if (!"numerator" %in% names(df)) df$numerator <- as.double(rep(NA_real_, nrow(df)))
  if (!"denominator" %in% names(df)) df$denominator <- as.double(rep(NA_real_, nrow(df)))
  df
}

# Apply standardization to relevant datasets
raw_data$ssi_incidence   <- standardize_counts(raw_data$ssi_incidence,   c("events_x","n_events","cases","events","n_x"), c("sample_size_n","n_total","total","denominator","n"))
raw_data$mortality       <- standardize_counts(raw_data$mortality,       c("n_deaths","events","deaths"),                c("n_total","total","denominator"))
raw_data$amr_proportions <- standardize_counts(raw_data$amr_proportions, c("events_x","n_events","resistant","non_susceptible"), c("sample_size_n","n_total","total","tested"))

message("Standardized numeric columns (numerator/denominator) for SSI, mortality, and AMR datasets.")
```
